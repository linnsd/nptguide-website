...
class EProviderDataTable extends DataTable
{
    ...
    public function dataTable($query, Request $request)
    {
        ...
        $dataTable = $dataTable
            ...
            })->filter(function ($eProvider) use ($request) {
                if ($request->search['value'] != null) {
                    $eProvider->where('name','like','%'.$request->search['value'].'%');
                }
                if ($request->p_type != null) {
                    $eProvider->where('e_provider_type_id', $request->p_type);
                }

                if ($request->tsh_id != null) {
                    $eProvider->where('tsh_id', $request->tsh_id);
                }

                if ($request->cat_id != null) {
                    $eProvider->where('category_id', $request->cat_id);
                }
            })
            ->addColumn('action', 'admin.e_providers.datatables_actions')
    ...
    public function query(EProvider $model): \Illuminate\Database\Eloquent\Builder
    {
        ...
        if (auth()->user()->hasRole('admin')) {
            return $model->newQuery()->with(["eProviderType", "townships", "category"])->where('e_providers.accepted', '1')->orderBy("e_providers.updated_at", "desc")->select("e_providers.*");
