...
//                }
    ...
    public function checkForUpdates(Request $request)
    {
        if (!config('installer.demo_app')) {
            ...
            if (!$containsUpdate) {
                return redirect(url('/admin/update/' . config('installer.currentVersion', 'v100')));
            }
        }
    ...
    public function index($type = null, $tab = null)
    {
        ...
        if ($type && $type === 'translation' && $tab) {
            ...
            $langFiles = $this->langManager->files();
            return view('admin.settings.' . $type . '.index', compact(['languages', 'type', 'tab', 'langFiles']));
        ...
        return view('admin.settings.' . $type . '.' . $tab . '', compact(['languages', 'type', 'tab', 'langFiles', 'timezones', 'upload', 'customFieldModels', 'currencies', 'mobileLanguages', 'containsUpdate']));
    }
